package com.cibertec.producto.config;

import io.swagger.v3.oas.models.OpenAPI;
import io.swagger.v3.oas.models.info.Contact;
import io.swagger.v3.oas.models.info.Info;
import io.swagger.v3.oas.models.servers.Server;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;

import java.util.List;

@Configuration
public class OpenApiConfig {

    @Bean
    OpenAPI productoOpenApi(@Value("${server.port:8081}") int port) {
        return new OpenAPI()
                .info(new Info()
                        .title("Producto Service API")
                        .description("Catálogo de productos y categorías del restaurante")
                        .version("1.0.0")
                        .contact(new Contact().name("Cibertec DSW2").email("comandas@cibertec.edu.pe")))
                .servers(List.of(new Server().url("http://localhost:" + port).description("Directo"),
                        new Server().url("http://localhost:8080/api").description("Vía API Gateway")));
    }
}
